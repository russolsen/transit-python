...
JSON_ESCAPED_CHARS = set([pyversion.unicode_f(c) for c in range(0x20)] + ["\\", "\n"])
...
class Marshaler(object):
    ...
    def emit_int(self, tag, i, rep, as_map_key, cache):
        #print("max_int", self.opts["max_int"])
        #print("i:", i, type(i))
...
class MsgPackMarshaler(Marshaler):
    ...
    def flush(self):
        #print("bytes:", type(self.packer.bytes()))
        #print("bytes:", self.packer.bytes())
...
class VerboseSettings(object):
    ...
    def emit_string(self, prefix, tag, string, as_map_key, cache):
        #print("emit string:", prefix, tag, type(tag), string, type(string))
